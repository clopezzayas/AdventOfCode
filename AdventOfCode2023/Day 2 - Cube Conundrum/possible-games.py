...
def get_possible_games():
    ...
    doc = open("game-input.txt", "r")
    
    game_id = 1 # simpler to keep track rather than grabbing it from the doc txt imo in each iteration.

    for line in doc:
        # Filter data and grab game half
        games_half = line.split(":")[-1]

        # For each game, ';' will act as delimiter
        list_of_games = games_half.split(";")

        is_valid_game = True
        
        for game in list_of_games: 
            
            # divide and iterate through each color to see if their value is less or equal
            # the allow max values per color.
            colors = game.split(",")  # i.e. [1 red, 2 green, 1 blue]
                
            for color_segment in colors: 

                color_number = int(color_segment.split()[0])
                color = color_segment.split()[1]

                if (color == "red" and color_number > max_reds) or \
                   (color == "blue" and color_number > max_blues) or \
                   (color == "green" and color_number > max_green):
                    is_valid_game = False
        if is_valid_game:
    ...
    for line in doc:
        ...
        for game in list_of_games: 
            ...
            for color_segment in colors: 
                ...
                if color == "red":
                   min_red_needed = max(min_red_needed, color_number)
                elif color == "blue":
                   min_blue_needed = max(min_blue_needed, color_number)
                elif color == "green":
                    min_green_needed = max(min_green_needed, color_number)

        min_cube_sum += (min_red_needed * min_blue_needed * min_green_needed)   
    return min_cube_sum
