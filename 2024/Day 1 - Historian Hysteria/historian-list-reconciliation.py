...
def get_distance_between_lists():
    # Result sum
    distance = 0

    # Input file with historian list
    doc = open("senior-historian-list.txt", "r")
    ...
    for line in doc:
        ...
        first_list.append(int(location_ids[0]))
        second_list.append(int(location_ids[1]))

    first_list.sort()
    second_list.sort()

    # Both lists are same size in test data
    for i in range(len(first_list)):
        distance += (abs(first_list[i]-second_list[i]))

    print(f"The distance between both lists is {distance}.")
    return distance
...
def get_similarity_score():
    ...
    for i in range(len(first_list)):
        item_frequency = second_list.count(first_list[i])

        if item_frequency != 0:
            similarity_score += (first_list[i]*item_frequency)

    print(f"The similarity score between both lists is {similarity_score}.")
